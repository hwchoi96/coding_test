...
FILE_PATH = 'samsung_input/samsung_snake_input.txt'
...
with open(FILE_PATH, 'r') as f:
    n_test = int(f.readline()[0])

    for n in range(n_test):
        map_size = int(f.readline().strip())
        apple_count = int(f.readline().strip())
        turn = [0] * 10001
        ...
        for a in range(apple_count):
            p1, p2 = f.readline().strip().split(' ')
            apple_point.append((p1, p2))

        snake_move_count = int(f.readline().split()[0])

        for i in range(snake_move_count):
            second, direction = f.readline().strip().split(' ')
