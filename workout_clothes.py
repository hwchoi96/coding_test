...
def solution(n, lost, reserve): 
    reser_not_dup = set(reserve) - set(lost)
    # 여벌 있는 친구 중, 체육복을 잃어버리지 않은 친구
    # ! 여벌이 있는데 잃어버렸으면, 그냥 걔는 자기꺼 입으면 되어서 고려할 필요 x
    
    lost_only = set(lost) - set(reserve)
    # 체육복 잃어버렸으면서, 여벌이 없는 친구
    
    # 여벌이 남는 친구들을 대상으로 greedy search
    for i in reser_not_dup: 
        if i - 1 in lost_only: 
